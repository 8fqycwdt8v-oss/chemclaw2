...
- **[substance-gate] Self-approvable controlled-substance override (HIGH — POLICY DECISION)** — `api/routes/chat.py` honors an override for *any* authenticated user who supplies a ≥20-char `override_justification`; `record_override` only writes an audit row, it does not gate on an approver. The actor the gate restricts can bypass it. Whether this is intended ("log-and-allow with accountability") or a bug depends on product policy. Options: require an elevated role for the override (fail closed for non-privileged), or make it a two-actor request→admin-approve flow. **Needs a product decision before changing the threat model.**
...
- **[auth] Missing JWKS-URL startup guard (LOW)** — `validate_auth_config` checks `CLERK_ISSUER` for prod but not `CLERK_JWKS_URL`/`CLERK_DOMAIN`; with neither set in prod the client silently targets `api.clerk.com/v1/jwks` and every token 401s (fails closed, but as a request-time outage rather than a startup failure). Mirror the issuer check.
...
- **[rate-limit] Empty-string identifier aliases onto `anon` bucket (LOW)** — `make_key` uses `if identifier` (falsy for `""`), so an empty id shares the anonymous bucket; use `if identifier is not None`. Not exploitable today (Clerk ids are non-empty).
