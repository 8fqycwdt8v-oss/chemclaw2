...
- **SSRF: DNS-rebinding TOCTOU** (`api/agent/tools.py:_assert_not_private`) — guard resolves the hostname, then `httpx` re-resolves at connect time, opening a short DNS-rebinding window between the two lookups. The pattern documented in CLAUDE.md §security-5 ("resolve once and bind the resolved IP") needs a custom `httpx.HTTPTransport` that pins the resolved IP and sets the `Host` header to the original hostname. Substantive change to the SSRF code — own PR. Multicast (224.0.0.0/4) is rejected as of this pass.
- **Stripe pattern in `_SECRET_PATTERNS`** (`api/agent/hooks.py`) — current `(sk|rk|pk)[-_][A-Za-z0-9]{20,}` does not match Stripe's `sk_live_…` / `pk_live_…` / `pk_test_…` keys because of the second underscore (`live`/`test` prefix segment). Add a Stripe-specific pattern: `(sk|rk|pk)_(live|test)_[A-Za-z0-9]{20,}`.
...
- **Extract fetch-with-redirect helper** (`api/agent/tools.py`) — `fetch_document` (lines 232-274) and `lookup_regulatory_guidance` (lines 603-653) re-implement the same redirect-revalidate loop. One shared helper reduces the SSRF audit surface to one place.
...
- **`patch_wiki_page` defense-in-depth** (`api/db/queries/wiki_write.py:252`) — the route currently checks ownership upstream, but the query function itself omits `created_by = :uid` in WHERE. A future caller could forget. Either add the predicate (signature change) or document the dependency loudly.
- **`fp_worker` imports `sqlalchemy.text`** — minor: workers should call `queries/*` per CLAUDE.md. Two SQL blocks would lift into a thin `queries/fingerprints.py`.
