...
- **Agent runtime broken: `@mcp.tool()` API mismatch** — `api/agent/tools.py` (and now `tools_chem.py`) decorates every tool with `@mcp.tool()` where `mcp = create_sdk_mcp_server(...)`. `claude-agent-sdk` 0.2.83 returns a `dict` from that call (`{'type': 'sdk', 'name': ..., 'instance': Server}`), and neither the dict nor the inner `Server` exposes a `.tool()` method. The SDK ships a top-level `@tool("name", "desc", {"arg": type})` decorator and passes the resulting list via `create_sdk_mcp_server(tools=[...])` instead. Calling `build_chemclaw_mcp_server(...)` raises `AttributeError: 'dict' object has no attribute 'tool'` on first agent invocation. No test exercises this path, which is why CI stayed green. Fix requires rewriting ~40 tool decorators to the `@tool(name, desc, schema)` form, building the tool list, and passing it to `create_sdk_mcp_server`. Pattern was introduced in commit `6d241bd0` (2026-05-17) when the codebase moved from one SDK version to another; the production agent path has been crashloop since. Priority is high if the chat surface is shipping; the refactor sweep keeps the broken pattern in place pending dedicated fix PR.
