...
@router.get("/api/todos/{session_id}", dependencies=[Depends(rate_limit("todos-read", 60))])
async def get_todos(
    session_id: str,
...
@router.put("/api/todos/{session_id}", dependencies=[Depends(rate_limit("todos-put", 30))])
...
@router.patch("/api/todos/{session_id}/{todo_id}", dependencies=[Depends(rate_limit("todos-patch", 60))])
