 ...
 * Runs immediately before TCP connect, so the IP we check is the IP we
...
// Singleton dispatcher shared across safeFetch calls. The connect-time IP
// range-check closes the TOCTOU window — the IP that passes is the IP
// undici opens a socket to.
const SAFE_DISPATCHER = new Agent({ connect: { lookup: safeLookup } });

/**
 * fetch() wrapper that:
 * 1. Validates initial URL hostname against caller-supplied allowlist
 * 2. Routes through an undici Agent whose lookup hook range-checks the
 *    resolved IP at connect time (closes the TOCTOU window)
 * 3. Re-validates the final hostname against the allowlist after redirects
 */
export async function safeFetch(
  ...
  // Pre-flight check — undici repeats this at connect time, but doing it
  // here gives a cleaner error for the common case (private IP in the URL).
